"""
Hippocampal Consolidation v14: Simplified CA1 (same motif as Sub)
==================================================================

CA1 uses the same dual-input convergence motif as every other stage:
  teaching (stellate via TA, fixed) + learning (CA3 via Schaffer, plastic)
  error = teaching - current -> update Schaffer weights

No plateau gating, no divisive normalization, no lateral inhibition.
Sparse stellate target (~4%) produces naturally sparse errors, so weight
updates are selective without any gating mechanism. Schaffer weights
start at zero (silent synapses) so only BTSP-written neurons fire.
...
class CA1:
    """
    CA1 learns to reproduce the stellate (sparse) signal from CA3 retrieval.

    Same dual-input convergence motif as Sub and every other stage:
      - Teaching: stellate via fixed TA weights (proximal, reality)
      - Learning: CA3 via plastic Schaffer weights (distal, retrieval)
      - Error = TA_activation - SC_activation
      - Schaffer weights updated to reduce error

    Stellate is ~4% active, so the error is naturally sparse: nonzero
    only at the ~4% of neurons that stellate drives. Updates are
    automatically selective without plateau gating.

    Schaffer weights start at zero (modeling silent synapses). Only
    neurons that have been trained will produce nonzero output.
    """
    def __init__(self, N_ca1, N_ca3, d_ec,
                 lr=0.3, ltd_rate=0.05, ltd_ca3_threshold=0.0,
    ...
    def encode(self, x_ca3, x_ec_stel):
        """
        Error-driven Hebbian learning. Same motif as Sub.
        Teaching: what stellate drives via TA.
        Current: what CA3 drives via Schaffer.
        """
        ...
        # Heterosynaptic LTD on inactive CA3 inputs
        ca3_inactive = (x_ca3 <= self.ltd_ca3_threshold).astype(float)
        # LTD proportional to how much the neuron was driven by teaching
        ta_active = (h_ta > 0).astype(float)
        self.W_sc *= (1.0 - self.ltd_rate * np.outer(ta_active, ca3_inactive))
    ...
    def retrieve(self, x_ca3, x_ec_stel=None):
        """
        Output = ReLU(Schaffer @ CA3). Sparsity from trained weights.
        ec_stel accepted but not used during retrieval (only for API compat).
        """
        h_sc = np.maximum(self.W_sc @ x_ca3, 0)

        # Compute mismatch if stellate available (for diagnostics)
        mismatch = 0.0
...
print("=" * 70)
print("HIPPOCAMPAL v14: SIMPLIFIED CA1, FiLM GATING")
print("=" * 70)
...
print(f"CA1({N_ca1}): TA=stellate, SC=CA3. Error-driven, no gating.")
...
with open("consolidation_v14_results.json", "w") as f:
    json.dump(results, f, indent=2)
print("  Saved to consolidation_v14_results.json")
...
fig.suptitle("Hippocampal v14: Simplified CA1, FiLM Gating",
...
ax.set_title("CA1 Learning (error-driven)"); ax.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig("consolidation_v14_results.png", dpi=150, bbox_inches='tight')
print("  Saved plot")
