"""
Hippocampal-Cortical Consolidation v6: Superposition Experiment
================================================================
...
Changes from v5:
  - CA1 fix #1: replaced global multiplicative weight decay with input-specific
    heterosynaptic LTD during BTSP plateau events. When a CA1 neuron undergoes
    a plateau potential, synapses from *inactive* CA3 inputs are multiplicatively
    depressed (pulled toward zero). This maintains per-neuron selectivity by
    actively erasing competing pattern information, counteracting the
    convergence-to-mean dynamic that caused CA1 representations to collapse
    (pairwise sim = 0.871, active frac = 1.0) in v5.
  - CA1 fix #2: added lateral inhibition during retrieval. Same iterative
    settle mechanism as DG (build_ring_inhibition + competitive dynamics),
    applied to Schaffer collateral activations at readout time. This enforces
    sparse population coding in CA1 output, so that only the most strongly
    driven neurons survive competition. Applied only during retrieve(), not
    during encode(), since BTSP plasticity is a dendritic event gated by
    plateau potentials, not by somatic spike output.
  - v5.1 (LTD only) showed that heterosynaptic LTD successfully maintained
    weight selectivity (feature decode R² improved from 0.677 to 0.894) but
    did not produce sparse output (active frac still 1.0, pairwise sim 0.80).
    The information was in the weights but not in the population vector.
    Lateral inhibition is the complementary mechanism that translates
    selective weights into selective output.
  - Biological motivation: CA1 has rich inhibitory circuitry (PV+ basket
    cells for perisomatic inhibition, bistratified cells, O-LM cells) that
    enforces competitive selection during retrieval/replay. The combination
    of heterosynaptic LTD (maintaining weight selectivity) and retrieval-time
    inhibition (enforcing sparse output) is what keeps CA1 representations
    pattern-discriminable despite ongoing plasticity.
  - Global weight decay removed (set to 1.0); LTD handles weight homeostasis.
...
print("=" * 70)
print("HIPPOCAMPAL-CORTICAL CONSOLIDATION v6: SUPERPOSITION EXPERIMENT")
print("=" * 70)
...
with open("consolidation_model_v3_results.json", "w") as f:
    json.dump(results, f, indent=2)

print("  Saved to consolidation_model_v3_results.json")
...
fig.suptitle("Hippocampal-Cortical Consolidation v6: Superposition Experiment",
...
plt.tight_layout()
plt.savefig("consolidation_model_v3_results.png", dpi=150, bbox_inches='tight')
print("  Saved plot to consolidation_model_v3_results.png")
