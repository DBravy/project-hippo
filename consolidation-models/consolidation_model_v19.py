"""
Hippocampal Consolidation v19: Pure Hebbian CA1 + Sub
======================================================

Both CA1 and Sub use pure Hebbian association:

  CA1: W_sc += lr * outer(stellate, ca3)
       Retrieval: k-WTA(50) for ~5% sparsity

  Sub: W_ca1 += lr * outer(pyramidal, ca1)
       Retrieval: k-WTA(500) for ~50% density

No error computation anywhere. No stellate connection to Sub.
Sub learns to produce pyramidal from CA1 output only.
FiLM at Vb: sigmoid(sub) * ca1.
"""
...
class CA1:
    """
    CA1 associates stellate activity with CA3 input via Hebbian learning.

    During encoding: stellate is directly imposed on CA1 (postsynaptic).
    CA3 arrives at Schaffer synapses (presynaptic). Hebbian rule:
      W_sc += lr * outer(stellate, ca3)

    The outer product is ~0.2% nonzero (4% stellate * 5% CA3), so each
    episode writes to a tiny, pattern-specific subset of the matrix.

    During retrieval: W_sc @ ca3 reactivates the stellate neurons that
    were associated with this CA3 pattern. k-WTA cleans up.

    Weight decay provides forgetting. No error computation.
    """
...
class Subiculum:
    """
    Learns to produce pyramidal (dense) from CA1 (sparse) via pure Hebbian.

    W_ca1 += lr * outer(pyramidal, ca1_output)

    Outer product is ~2.5% nonzero (50% pyramidal * 5% CA1).
    Slower lr than CA1 (conventional LTP vs BTSP-like).
    k-WTA at 500 to match pyramidal density (~50%).
    Zero-initialized weights (silent synapses).
    """
    def __init__(self, N_sub, N_ca1, d_ec,
                 lr=1.0, connectivity_prob=0.33, k_active=500):
        self.N_sub = N_sub
        self.lr = lr
        self.k_active = k_active
...
ca1_params = {
    "lr": 50.0, "weight_decay": 1.000, "k_active": 50,
}
...
print("=" * 70)
print("HIPPOCAMPAL v19: PURE HEBBIAN CA1+SUB, FiLM GATING")
print("=" * 70)
...
print(f"CA1(1000): pure Hebbian + k-WTA(50) + decay.")
print(f"Sub({N_sub}): pure Hebbian + k-WTA(500).")
...
with open("consolidation_v19_results.json", "w") as f:
    json.dump(results, f, indent=2)
print("  Saved to consolidation_v19_results.json")
...
fig.suptitle("Hippocampal v19: Pure Hebbian CA1+Sub, FiLM Gating",
...
ax.set_title("CA1 Learning (Hebbian)"); ax.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig("consolidation_v19_results.png", dpi=150, bbox_inches='tight')
print("  Saved plot")
