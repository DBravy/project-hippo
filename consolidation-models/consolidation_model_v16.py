"""
Hippocampal Consolidation v16: Direct Teaching Targets (no random projections)
===============================================================================

Since CA1, Sub, and EC are all 1000-dim, random projections between them
just add noise. Teaching signals are used directly:

  CA1: error = stellate - ReLU(W_sc @ ca3)
       Stellate is 4% sparse, so the error is naturally sparse.
       No W_ta projection, no plateau gating needed.

  Sub: error = (pyramidal + 0.2*stellate) - ReLU(W_ca1 @ ca1)
       Teaching target is the combined dense+weak sparse signal directly.
       No W_ec_pyr or W_ec_stel projections.

  DG is the exception: its random projection is part of its computational
  role (creating a different basis for pattern separation).

CA1 has weight decay (depression-dominated) for forgetting.
Sub has minimal LTD + row-norm clipping for stability.
FiLM gating at Vb: sigmoid(sub) * ca1.
"""
...
class CA1:
    ...
    No W_ta projection. Since both CA1 and stellate are 1000-dim,
    the teaching target IS the stellate vector. Each CA1 neuron i
    should be active when stellate neuron i is active.

    error = stellate - ReLU(W_sc @ ca3)
    This error is ~4% sparse (from stellate), so updates are naturally
    selective without any gating mechanism.

    Weight decay provides depression-dominated dynamics: old associations
    fade, only recent/consistent patterns survive.
    """
    ...
    def retrieve(self, x_ca3, x_ec_stel=None):
        """ReLU(Schaffer @ CA3). Stellate optional (for mismatch diagnostic)."""
        h_sc = np.maximum(self.W_sc @ x_ca3, 0)
        mismatch = 0.0
...
print("=" * 70)
print("HIPPOCAMPAL v16: DIRECT TEACHING TARGETS, FiLM GATING")
print("=" * 70)
...
print(f"CA1(1000): direct stellate target + weight decay.")
...
with open("consolidation_v16_results.json", "w") as f:
    json.dump(results, f, indent=2)
print("  Saved to consolidation_v16_results.json")
...
fig.suptitle("Hippocampal v16: Direct Teaching Targets, FiLM Gating",
...
ax.set_title("CA1 Learning (direct target)"); ax.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig("consolidation_v16_results.png", dpi=150, bbox_inches='tight')
print("  Saved plot")
