"""
Hippocampal Consolidation v12: CA1 TA from Stellate, Dense Reconstructed at Sub
=================================================================================

Key insight: EC L2 pyramidal cells (inhibitory at CA1) are the functional
inverse of stellate cells. Dense TA excitation minus inverse-sparse inhibition
= the sparse stellate signal. Rather than modeling the indirect subtraction,
CA1's TA target IS the stellate signal directly.

This means CA1 learns to reproduce the sparse, episode-specific representation
from CA3 retrieval. Plateau gating is extremely selective (~4% of CA1 neurons
per episode), minimizing cross-episode interference. Sub then reconstructs
the dense component from CA1's sparse output.
...
Architecture:
  ...
  EC Superficial (II/III):
    Stellate (sparse, ~4% active, lateral inh)
    Pyramidal (dense, ~50% active, no inh)
    Pyramidal excites stellate (not vice versa)
        |
        +--- Stellate ---> DG(1000) ---> CA3(1000)  [storage]
        +--- Stellate ---> CA1 TA (teaching signal)
        +--- Pyramidal --> Sub (strong teaching)
        +--- Stellate ---> Sub (weak teaching)
        |
  CA3 retrieval ---> CA1 Schaffer (plastic)
  CA1: learns to produce stellate from CA3. Sparse output (~4%).
        |
  CA1 --------> Sub (plastic, slow)
  Sub: learns to produce pyramidal/dense from CA1 sparse output.
        |
  EC Deep Vb: FiLM = sigmoid(sub_dense) * ca1_sparse
"""
...
class CA1:
    """
    CA1 learns to reproduce the stellate (sparse) representation from CA3.

    TA pathway carries the stellate signal directly. Since stellate is ~4%
    active, plateau gating is extremely selective: only ~4% of CA1 neurons
    undergo plateau potentials per episode. This means each memory only
    modifies a tiny subset of Schaffer weights, minimizing interference.

    Lateral inhibition during retrieval further enforces sparsity, but
    the primary sparsity comes from the TA target itself being sparse.
    """
    ...
    def compute_activations(self, x_ca3, x_ec_stel):
        """TA uses stellate (sparse) signal."""
        h_ta_raw = np.maximum(self.W_ta @ x_ec_stel, 0)
    ...
    def encode(self, x_ca3, x_ec_stel):
        """BTSP: plateau gating from stellate. Very selective (~4% gated)."""
    ...
    def retrieve(self, x_ca3, x_ec_stel):
        """Retrieval with lateral inhibition."""
        h_ta, h_sc, gate, h_ta_raw, h_sc_raw = self.compute_activations(x_ca3, x_ec_stel)
        gated_error = gate * (h_ta - h_sc)
        mismatch = float(np.linalg.norm(gated_error) / (np.linalg.norm(h_ta) + 1e-10))
...
print("=" * 70)
print("HIPPOCAMPAL v12: CA1 TA FROM STELLATE, FiLM GATING")
print("=" * 70)
...
print(f"CA1({N_ca1}): TA=stellate, SC=CA3. Target is sparse.")
...
with open("consolidation_v12_results.json", "w") as f:
    json.dump(results, f, indent=2)
print("  Saved to consolidation_v12_results.json")
...
fig.suptitle("Hippocampal v12: CA1 TA=Stellate, FiLM Gating",
...
ax.set_title("CA1 Learning (TA=stellate)"); ax.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig("consolidation_v12_results.png", dpi=150, bbox_inches='tight')
print("  Saved plot")
