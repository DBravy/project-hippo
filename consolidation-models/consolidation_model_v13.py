"""
Hippocampal Consolidation v13: CA1 without lateral inhibition
==============================================================

CA1's job is simple: reproduce the stellate (sparse) representation from
CA3 retrieval. BTSP trains Schaffer weights with highly selective plateau
gating (~4% of neurons per episode). The trained weights directly produce
sparse output via ReLU. No lateral inhibition needed -- sparsity comes
from the weights themselves, which were trained against a sparse target.

Sub then learns the dense (pyramidal) component from CA1's sparse output.
FiLM gating at Vb: sigmoid(sub_dense) * ca1_sparse.
"""
...
class CA1:
    ...
    TA carries the stellate signal. Plateau gating is extremely selective
    (~4% of neurons per episode). Schaffer weights learn to produce, from
    CA3 input, the same sparse activation that stellate drives via TA.

    No lateral inhibition. The trained Schaffer weights directly produce
    sparse output via ReLU. Sparsity is a consequence of the weights
    having been trained against a sparse target, not imposed by inhibition.
    """
    def __init__(self, N_ca1, N_ca3, d_ec,
        ...
        self.W_sc = np.zeros((N_ca1, N_ca3)) * mask
    ...
    def retrieve(self, x_ca3, x_ec_stel):
        """
        Retrieval: Schaffer-driven activation through ReLU.
        No lateral inhibition. Sparsity comes from the weights.
        """
        h_ta, h_sc, gate, h_ta_raw, h_sc_raw = self.compute_activations(x_ca3, x_ec_stel)
        gated_error = gate * (h_ta - h_sc)
        mismatch = float(np.linalg.norm(gated_error) / (np.linalg.norm(h_ta) + 1e-10))

        # Direct Schaffer output, no inhibition
        h_out = h_sc_raw
...
print("=" * 70)
print("HIPPOCAMPAL v13: CA1 NO LATERAL INHIBITION, FiLM GATING")
print("=" * 70)
...
print(f"CA1({N_ca1}): TA=stellate, SC=CA3. No lateral inhibition.")
...
with open("consolidation_v13_results.json", "w") as f:
    json.dump(results, f, indent=2)
print("  Saved to consolidation_v13_results.json")
...
fig.suptitle("Hippocampal v13: CA1 No Lateral Inhibition, FiLM Gating",
...
ax.set_title("CA1 Learning (no lat. inh.)"); ax.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig("consolidation_v13_results.png", dpi=150, bbox_inches='tight')
print("  Saved plot")
