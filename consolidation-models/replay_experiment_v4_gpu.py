"""
Replay Experiment v4 (GPU): Balanced Replay
=============================================

Same hippocampus and drifting data as v2, but with replay as a
real training signal:
  - Same optimizer (AdamW) for both training and replay
  - Same batch size for both
  - Alternating: one training batch, one replay batch
  - Replay updates only layers >= replay_layer (upper layers frozen
    during replay via selective parameter group)

This makes replay equally weighted with direct experience,
which is a fairer test of whether it helps.
"""
...
# =============================================================================
# 1. HIPPOCAMPAL SYSTEM (ported from NumPy to PyTorch)
# =============================================================================
...
class CA3:
    def __init__(self, N, k_active, device='cpu', dtype=torch.float32):
        self.N = N
...
class HippocampalSystem:
    def __init__(self, d_ec, D_dg, N_ca3, N_ca1, k_ca3,
        ...
        self.dg = DentateGyrusLateral(d_ec, D_dg, **(dg_params or {}),
                                      device=device, dtype=dtype)
        self.ca3 = CA3(N_ca3, k_ca3, device=device, dtype=dtype)
...
# =============================================================================
# 2. REPLAY MEMORY (ported to torch)
# =============================================================================

class ReplayMemory:
    def __init__(self, d_model, d_ec, hippo_kwargs, W_in,
                 recency_tau=5000, max_age=None,
                 device='cpu', dtype=torch.float32):
        """
        Args:
            recency_tau: Exponential decay time constant (in storage steps).
                         Smaller values = stronger recency bias.
                         A memory's replay probability decays as exp(-age / tau).
            max_age:     Hard cutoff. Memories older than this many steps are
                         never replayed. None = no cutoff (infinite tail).
        """
        ...
        self.W_in = torch.tensor(W_in, device=device, dtype=dtype)
        self.hippo = HippocampalSystem(**hippo_kwargs, device=device, dtype=dtype)
        self.raw_h4 = []
        self.raw_targets = []
        self.gated_outputs = []
        self.store_steps = []          # timestamp (global step) for each memory
        self._global_step = 0          # monotonic counter incremented on each store()
        self.W_out = torch.zeros((d_model, d_ec), device=device, dtype=dtype)
        self.b_out = torch.zeros(d_model, device=device, dtype=dtype)
        self._fitted = False
        self.n_stored = 0

    @torch.no_grad()
    def store(self, h4, target):
        ec = self.W_in @ h4
        mm = self.hippo.encode_single(ec)
        gated, _ = self.hippo.recall(ec)
        self.raw_h4.append(h4.clone())
        self.raw_targets.append(target.clone())
        self.gated_outputs.append(gated.clone())
        self.store_steps.append(self._global_step)
        self._global_step += 1
        self.n_stored += 1
        return mm
    ...
    def refit_output_projection(self):
        if self.n_stored < 50:
            return
        X = torch.stack(self.gated_outputs)
        Y = torch.stack(self.raw_h4)
        lam = 0.01
        XtX_reg = X.T @ X + lam * torch.eye(X.shape[1], device=self.device, dtype=self.dtype)
        XtY = X.T @ Y
        W_out_T = torch.linalg.solve(XtX_reg, XtY)
        self.W_out = W_out_T.T
        self.b_out = Y.mean(dim=0) - self.W_out @ X.mean(dim=0)
        self._fitted = True

    def _recency_sample(self, n):
        """Sample n indices with probability proportional to exp(-age / tau).

        If max_age is set, memories older than max_age steps are excluded
        entirely (hard forgetting).  Returns a numpy array of indices.
        """
        steps = np.array(self.store_steps, dtype=np.float64)
        ages = self._global_step - steps          # age 0 = just stored

        # Hard cutoff
        if self.max_age is not None:
            eligible = ages <= self.max_age
        else:
            eligible = np.ones(len(ages), dtype=bool)

        if eligible.sum() < n:
            # Not enough eligible memories; fall back to all
            eligible = np.ones(len(ages), dtype=bool)

        weights = np.exp(-ages / self.recency_tau)
        weights[~eligible] = 0.0
        total = weights.sum()
        if total < 1e-30:
            # Degenerate case: uniform fallback
            probs = np.ones(self.n_stored) / self.n_stored
        else:
            probs = weights / total

        return np.random.choice(self.n_stored, size=n, replace=False, p=probs)

    @torch.no_grad()
    def recall_hippo(self):
        idx = self._recency_sample(1)[0]
        raw = self.raw_h4[idx]
        ec = self.W_in @ raw
    ...
    @torch.no_grad()
    def recall_hippo_batch(self, batch_size):
        """Recall a batch of hippocampal reconstructions (recency-weighted)."""
        indices = self._recency_sample(batch_size)
        h4_list = []
        tgt_list = []
        for idx in indices:
            raw = self.raw_h4[idx]
            ...
            gated, _ = self.hippo.recall(ec)
            recon = self.W_out @ gated + self.b_out
            h4_list.append(recon)
            tgt_list.append(self.raw_targets[idx])
        return torch.stack(h4_list), torch.stack(tgt_list)
    ...
    @torch.no_grad()
    def recall_oracle_batch(self, batch_size):
        """Recall a batch of exact stored (layer4, target) pairs (recency-weighted)."""
        indices = self._recency_sample(batch_size)
        h4_list = [self.raw_h4[i] for i in indices]
        tgt_list = [self.raw_targets[i] for i in indices]
        return torch.stack(h4_list), torch.stack(tgt_list)
...
# =============================================================================
# 4. TASK: drifting data across epochs (unchanged — stays NumPy for data gen)
# =============================================================================
...
def train_condition(
    ...
    replay_layer, replay_mode, replay_lr=None, replay_start_epoch=100,
    ...
    # Separate optimizer for replay steps (uses replay_lr, only updates upper layers)
    if replay_lr is None:
        replay_lr = lr
    replay_optimizer = torch.optim.AdamW(
        cortex.upper_parameters(replay_layer), lr=replay_lr, weight_decay=weight_decay)

    replay_batch_size = batch_size // 2

    log = {"val": [], "train": [], "replay_loss": [], "recon_quality": []}
    global_step = 0
    min_stored = replay_batch_size * 2
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(0, samples_per_epoch - batch_size + 1, batch_size):
            ...
            # === Normal training step (full forward, all layers) ===
            ...
            if replay_mem is not None:
                h4 = states[replay_layer][0].detach()
                tgt = y[0].detach()
            ...
            # === Replay step (half batch_size, starts after warmup) ===
            if (replay_mode != 'none'
                    and replay_mem is not None
                    and replay_mem._fitted
                    and replay_mem.n_stored >= min_stored
                    and epoch >= replay_start_epoch):

                if replay_mode == 'hippo':
                    h4_batch, y_batch = replay_mem.recall_hippo_batch(replay_batch_size)
                elif replay_mode == 'oracle':
                    h4_batch, y_batch = replay_mem.recall_oracle_batch(replay_batch_size)
                else:
                    raise ValueError(f"Unknown: {replay_mode}")

                # Forward from replay_layer only (lower layers don't participate)
                replay_optimizer.zero_grad()
                pred_replay = cortex.forward_from_layer(h4_batch, replay_layer)
                r_loss = F.mse_loss(pred_replay, y_batch)
                r_loss.backward()
                torch.nn.utils.clip_grad_norm_(
                    cortex.upper_parameters(replay_layer), max_norm=1.0)
...
def main():
    ...
    replay_layer = 4
    replay_lr = 1e-4            # Learning rate for replay steps (None = same as lr)
    output_refit_interval = 200
    drift_rate = 0.05  # How much inputs change per epoch

    # Skip conditions
    skip_condition_1 = False     # No replay
    skip_condition_3 = False     # Oracle replay

    # Recency-weighted replay parameters
    recency_tau = 1000   # Exponential decay constant (in storage steps).
                         # With 2000 samples/epoch storing 1 per batch step,
                         # 5000 ~ a few epochs worth of half-life.
    max_age = None       # Hard forgetting cutoff (None = keep all)
    ...
    # Shared input projection (built in NumPy, converted to torch in ReplayMemory)
    ...
    # Use numpy cosine_sim for the drift check
    ...
    print("=" * 70)
    print("Replay Experiment v4: Balanced Replay")
    print("=" * 70)
    ...
    print(f"Replay: layer={replay_layer}, lr={replay_lr}, starts epoch 100, batch={batch_size//2} (half normal)")
    print(f"Recency: tau={recency_tau}, max_age={max_age}")
    if skip_condition_1 or skip_condition_3:
        skipped = []
        if skip_condition_1:
            skipped.append("1 (no replay)")
        if skip_condition_3:
            skipped.append("3 (oracle)")
        print(f"Skipping conditions: {', '.join(skipped)}")
    print(f"Hippo: d_ec={d_ec}, k_ca3=50 (GPU-accelerated)")
    ...
    if not skip_condition_1:
        ...
        mem_none = ReplayMemory(d_model, d_ec, hippo_kwargs, shared_W_in,
                                recency_tau=recency_tau, max_age=max_age,
                                device=device)
        ...
        print(f"\n  Skipping condition 1 (no replay)")
    ...
    log_hippo = train_condition(
        ...
        n_epochs, samples_per_epoch, batch_size, lr, weight_decay,
        replay_layer,
        replay_mode='hippo', replay_lr=replay_lr,
        output_refit_interval=output_refit_interval)

    # =================================================================
    # CONDITION 3: Oracle replay
    # =================================================================
    log_oracle = None
    if not skip_condition_3:
        print(f"\n{'='*70}")
        ...
        log_oracle = train_condition(
            ...
            n_epochs, samples_per_epoch, batch_size, lr, weight_decay,
            replay_layer,
            replay_mode='oracle', replay_lr=replay_lr,
            output_refit_interval=output_refit_interval)
    else:
        print(f"\n  Skipping condition 3 (oracle replay)")
    ...
    final_none = log_none["val"][-1] if log_none else None
    final_hippo = log_hippo["val"][-1]
    final_oracle = log_oracle["val"][-1] if log_oracle else None
    best_none = min(log_none["val"]) if log_none else None
    best_hippo = min(log_hippo["val"])
    best_oracle = min(log_oracle["val"]) if log_oracle else None

    parts_final = [f"hippo={final_hippo:.6f}"]
    parts_best = [f"hippo={best_hippo:.6f}"]
    if final_none is not None:
        parts_final.insert(0, f"none={final_none:.6f}")
        parts_best.insert(0, f"none={best_none:.6f}")
    if final_oracle is not None:
        parts_final.append(f"oracle={final_oracle:.6f}")
        parts_best.append(f"oracle={best_oracle:.6f}")

    print(f"  Final val:  {', '.join(parts_final)}")
    print(f"  Best val:   {', '.join(parts_best)}")

    if log_hippo["recon_quality"]:
        print(f"  Final hippo recon: {log_hippo['recon_quality'][-1]:.4f}")

    if best_none is not None and best_none > 0:
    ...
    results = {
        "params": {
            ...
            "lr": lr, "weight_decay": weight_decay, "replay_lr": replay_lr,
            "replay_layer": replay_layer, "d_ec": d_ec, "drift_rate": drift_rate,
            "recency_tau": recency_tau, "max_age": max_age,
        },
        "hippo_replay": {
        ...
        "summary": {
            "final_val": {"hippo": final_hippo},
            "best_val": {"hippo": best_hippo},
        },
    }
    if log_none is not None:
        results["no_replay"] = {"val": log_none["val"], "train": log_none["train"]}
        results["summary"]["final_val"]["none"] = final_none
        results["summary"]["best_val"]["none"] = best_none
    if log_oracle is not None:
        results["oracle_replay"] = {
            "val": log_oracle["val"], "train": log_oracle["train"],
            "replay_loss": log_oracle["replay_loss"],
        }
        results["summary"]["final_val"]["oracle"] = final_oracle
        results["summary"]["best_val"]["oracle"] = best_oracle

    with open("replay_experiment_v4_gpu_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\n  Saved to replay_experiment_v4_gpu_results.json")

    # =================================================================
    # PLOT
    # =================================================================
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle("Replay Experiment v4: Balanced Replay (Same Optimizer, Same Batch Size)",
    ...
    plt.tight_layout()
    plt.savefig("replay_experiment_v4_gpu_results.png", dpi=150, bbox_inches='tight')
    print("  Saved to replay_experiment_v4_gpu_results.png")
