"""
Replay Experiment v3 (GPU): Balanced Replay
=============================================
...
class ReplayMemory:
    def __init__(self, d_model, d_ec, hippo_kwargs, W_in, device='cpu', dtype=torch.float32):
    ...
    def recall_hippo_batch(self, batch_size):
        """Recall a batch of hippocampal reconstructions."""
        indices = np.random.choice(self.n_stored, size=batch_size, replace=False)
        h4_list = []
        tgt_list = []
        for idx in indices:
            raw = self.raw_h4[idx]
    ...
    def recall_oracle_batch(self, batch_size):
        """Recall a batch of exact stored (layer4, target) pairs."""
        indices = np.random.choice(self.n_stored, size=batch_size, replace=False)
...
def train_condition(
    ...
    replay_layer, replay_mode, replay_start_epoch=100,
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(0, samples_per_epoch - batch_size + 1, batch_size):
            ...
            if (replay_mode != 'none'
                ...
                # Forward from layer 4 only (layers 0-3 don't participate)
                main_optimizer.zero_grad()
...
def main():
    ...
    print("=" * 70)
    print("Replay Experiment v3: Balanced Replay")
    print("=" * 70)
    ...
    print(f"Replay: layer={replay_layer}, starts epoch 100, batch={batch_size//2} (half normal)")
    ...
    with open("replay_experiment_v3_gpu_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\n  Saved to replay_experiment_v3_gpu_results.json")

    # =================================================================
    # PLOT
    # =================================================================
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle("Replay Experiment v3: Balanced Replay (Same Optimizer, Same Batch Size)",
    ...
    plt.tight_layout()
    plt.savefig("replay_experiment_v3_gpu_results.png", dpi=150, bbox_inches='tight')
    print("  Saved to replay_experiment_v3_gpu_results.png")
