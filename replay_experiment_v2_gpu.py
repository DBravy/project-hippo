"""
Replay Experiment v2 (GPU): Drifting Data (Rhyme but Not Repeat)
================================================================

Same as replay_experiment_v2.py but with the hippocampal system ported
from NumPy to PyTorch so it runs on GPU alongside the cortex.

All hippocampal operations use torch tensors on the same device as the
cortex, eliminating CPU-GPU transfers in the training loop. Hebbian
updates run under torch.no_grad() (no autograd graph needed).
...
def train_condition(
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(0, samples_per_epoch - batch_size + 1, batch_size):
            ...
            # === Store in hippocampus (tensors stay on device) ===
            ...
            if (replay_mode != 'none'
                ...
                h4_t = h4_replay.unsqueeze(0)
                y_t = y_replay.unsqueeze(0)
...
def main():
    ...
    print("=" * 70)
    print("Replay Experiment v2 (GPU): Drifting Data (Rhyme but Not Repeat)")
    print("=" * 70)
    ...
    with open("replay_experiment_v2_gpu_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\n  Saved to replay_experiment_v2_gpu_results.json")

    # =================================================================
    # PLOT
    # =================================================================
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle("Replay Experiment v2 (GPU): Drifting Data (Rhyme but Not Repeat)",
    ...
    plt.tight_layout()
    plt.savefig("replay_experiment_v2_gpu_results.png", dpi=150, bbox_inches='tight')
    print("  Saved to replay_experiment_v2_gpu_results.png")
